      #####  #####  #####  #####  #####  #####  #####  #####  #####  #####  #####
      #     APLICACION SHINY REALIZADA POR: NICOLAS FERREIRA | LUIS GAGNEVIN    #
      #                                                                         #
      #         NUEVAS TECNOLOGIAS PARA EL ANALISIS ESTADISTICO DE DATOS        #
      #                                                                         #
...
# DATOS GLOBALES PARA MAPAMUNDI
library(sf)
library(rnaturalearth)
library(rnaturalearthdata)
library(rgeos)

# LECTURA | MANEJO DE DATOS
library(lubridate)
library(DT)
library(tidyverse)
library(readr)
...
contenido <- dashboardBody(
  tags$head(tags$link(rel = "icon",type="image/png",sizes="64x64", 
                      href = "https://www.pacificasalud.com/Imagenes/Noticias/1584720749040-8e6a79b0896035f105f65732c2617dcd.png")),
  ...
  tabItems(
    tabItem(tabName = "inicio", h1("Nuevas tecnologías para el análisis estadístico de datos"),
            ...
            h3("Introducción | Coronavirus"),
                                             p("Coronavirus es una familia de virus conocidos por causar enfermedades que afectan las vías respiratorias de las personas, 
                                                en mayor manera a la población de más edad y/o con enfermedades respiratorias o pobres condiciones físicas."),
            
                                             p("EL COVID-19 es la enfermadad causada por el coronavirus y nos ha afectado de gran manera en estos años. 
                                                La misma se originó a fines del 2019 con una infección en China y fue declarada pandemia por la OMS el 30 de Enero de 2020, 
                                                transformándose en una de las crisis sanitarias que más ha afectado a la humanidad."),
           
            HTML("<br><br>"),
            
            h4("Objetivo de la aplicación"), 
                                             p("El objetivo principal de la aplicación es generar un informe interactivo que permita visualizar el efecto del COVID-19 en la población mundial, 
                                                haciendo énfasis en América y la región. 
                                             ...
                                             p("Para lograr que la app sea eficiente en términos de recursos, utilizamos programación reactiva, generando outputs que dependen de los inputs. 
                                                Además de esto, se programaron diagramas y gráficos dinámicos, donde el usuario podrá interactuar e interpretar los resultados rápidamente, 
                                                observando por su propia cuenta
                                                como ha sido la evolución del virus"),
            
            HTML("<br>"),
            
            h4("Informe"),
                                             p("Contamos adicionalmente con un informe donde se detalla información relevante que servirá de ayuda al momento de visualizar la aplicación e interpretar los gráficos y datos.",
            ...
            h4("Desarrollo"), 
                                             p("La aplicación se desarrolló utilizando el lenguaje de ciencia de datos: R, junto con la aplicacion RStudio, 
            ...
            h4("Datos Utilizados"),
                                             p("Los datos utilizados fueron obtenidos de la plataforma", 
                                                a(href="https://data.world/covid-19-data-resource-hub/covid-19-case-counts/workspace/file?filename=COVID-19+Activity.csv","data.world"), 
                                                ...
                                                "los cuáles contienen información sobre la vacunación en Uruguay y son actualizados diariamente con datos del",
                                                 a(href="https://monitor.uruguaysevacuna.gub.uy/","monitor de vacunacion")),
...
server <- function(input, output, session) {
  ...
  datos_Por_Fecha_ts <- reactive(
                        xts(x=datos_Por_Fecha()[,3:5],
  ...
  maximo<-reactive(
          datospais()%>%filter(PEOPLE_POSITIVE_NEW_CASES_COUNT==max(PEOPLE_POSITIVE_NEW_CASES_COUNT))
   )
  
  
  paisgraf<-reactive(
            ggplot(datospais(),aes(x=REPORT_DATE,y=PEOPLE_POSITIVE_NEW_CASES_COUNT))+
            geom_line()+
            geom_point(aes(x=maximo()$REPORT_DATE,
                           y=maximo()$PEOPLE_POSITIVE_NEW_CASES_COUNT),color="red")+
              
            geom_text(aes(x=maximo()$REPORT_DATE,
                          y=maximo()$PEOPLE_POSITIVE_NEW_CASES_COUNT),
  ...
  #  SELECCIONA FECHA DEL PICO
  maximo2<- reactive(
            datospais()%>%filter(PEOPLE_DEATH_NEW_COUNT==max(PEOPLE_DEATH_NEW_COUNT))
  )
  
  
  paisgraf2<- reactive(
              ggplot(datospais(),aes(x=REPORT_DATE,y=PEOPLE_DEATH_NEW_COUNT))+
              geom_line()+
              geom_point(aes(x=maximo2()$REPORT_DATE,
                             y=maximo2()$PEOPLE_DEATH_NEW_COUNT),color="red")+
                
              geom_text(aes(x=maximo2()$REPORT_DATE,
                            y=maximo2()$PEOPLE_DEATH_NEW_COUNT),
  ...
  fig1<-reactive(
              ...
              geom_line(aes(x=REPORT_DATE, 
                            y=PEOPLE_DEATH_NEW_COUNT, 
                            color = "Fallecidos diariamente")) +
                
              geom_vline(aes(xintercept= ymd("2021-02-27"),
                             color="Comienzo de vacunación"), 
                             linetype="dashed", size= 1)+
                
    geom_text(aes(x=ymd("2021-02-27"),y=0),
              label=paste0("2021-02-27"),
              color="#d95f02",
              vjust=1.5, hjust=1.1)+
  ...
  popregion <- reactive(
        ...
        group_by(Location)%>%
        summarise(infeccion=round((PEOPLE_POSITIVE_CASES_COUNT/(PopTotal*1000)),digits=2), 
                  REPORT_DATE,letal=round((PEOPLE_DEATH_COUNT/PEOPLE_POSITIVE_CASES_COUNT),digits=2))
  ...
  tasagraf<- reactive(
        ...
        geom_line(aes(x=REPORT_DATE, 
                      y=infeccion, 
                      color = "Tasa de infección")) +
          
        geom_line(aes(x=REPORT_DATE, 
                      y=letal, 
                      color = "Tasa de letalidad")) + 
  ...
  output$mapa<-renderGirafe(girafe(ggobj = graf(),
                                   width_svg = 10, 
                                   height_svg = 3, 
  ...
  output$casospais<- renderPlotly({paisgraf()})
  output$muertespais<- renderPlotly({paisgraf2()})
